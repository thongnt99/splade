...
class Sparse2Dense(nn.Module):
    ...
    def __init__(self, sparse_model_name_or_path: str, dense_model_name_or_path: str, max_seq_length: Optional[int] = None,
        ...
        self.dense_tokenizer = AutoTokenizer.from_pretrained(dense_model_name_or_path, cache_dir=cache_dir, **tokenizer_args)
        self.mean_pooling = torch.nn.DataParallel(MeanPooling(768)) 
        self.sparse_to_dense = torch.nn.DataParallel(torch.nn.Linear(self.get_word_embedding_dimension(), 768))
            # Dense2Sparse(model_type=model_type, out_dim=self.get_word_embedding_dimension()))
        self.max_seq_length = max_seq_length

    def __repr__(self):
        return "Sparse2Dense ({}) with Transformer model: {} {}".format(self.get_config_dict(), self.sparse_model.__class__.__name__, self.dense_model.__class__.__name__)
    ...
    def forward(self, features):
       ...
       # covert sparse to dense 
        dense_from_sparse = self.sparse_to_dense(features["sentence_embedding"])
        features.update({"dense_from_sparse": dense_from_sparse})
        return features
    ...
    def save(self, output_path: str):
        # self.auto_model.module.save_pretrained(output_path)
        # self.tokenizer.save_pretrained(output_path)
        torch.save(self.sparse_to_dense.module.state_dict(), f"{output_path}/dense_to_sparse.pt")
        with open(os.path.join(output_path, 'sentence_bert_config.json'), 'w') as fOut:
            json.dump(self.get_config_dict(), fOut, indent=2)
