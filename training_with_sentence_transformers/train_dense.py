...
model_save_path = f'output/dense{args.lambda_q}_{args.lambda_d}_{model_name.replace("/", "-")}-batch_size_{train_batch_size}-{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}'
...
# Train the model
model.fit(train_objectives=[(dense_train_dataloader, dense_loss)],
          epochs=num_epochs,
