...
parser.add_argument("--extra_vocab", default=10000, type=int)
...
word_embedding_model = models.TransformerExtraVocab(model_name, max_seq_length=max_seq_length, extra_vocab=args.extra_vocab)
model = SentenceTransformer(modules=[word_embedding_model])

model_save_path = f'output/extra_{args.extra_vocab}_{args.lambda_q}_{args.lambda_d}_{model_name.replace("/", "-")}-batch_size_{train_batch_size}-{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}'
