...
word_embedding_model = models.Sparse2Dense(sparse_model_name, dense_model_name, max_seq_length=max_seq_length)
model = SentenceTransformer(modules=[word_embedding_model])

model_save_path = f'output/sparse2dense_{sparse_model_name.replace("/","-")}-{dense_model_name.replace("/","-")}-lambda_rank_{args.lambda_rank}-lambda_rec_{args.lambda_rec}-batch_size_{train_batch_size}-{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}'
...
train_loss = losses.Sparse2Dense(model=model, lambda_rank=args.lambda_rank, lambda_rec=args.lambda_rec)
